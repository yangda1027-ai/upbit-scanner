...
def calc_transition(rs, latest, now, change_rate_24h):
    ...
    # 단계별 점수: precursor 35 + reignition 30 + current confirmation 35
    score = 0.0
    score += clamp(precursor_best / 100, 0, 1) * 18
    score += clamp(precursor_count / 3, 0, 1) * 10
    score += clamp(precursor_abc_n / 2, 0, 1) * 7

    score += clamp((max_prior_vr - 2) / 20, 0, 1) * 10
    score += clamp((max_prior_a15 - 1) / 8, 0, 1) * 10
    score += clamp((max_prior_a30 - 1) / 5, 0, 1) * 7
    score += clamp(reignition_count / 3, 0, 1) * 3

    score += clamp(latest_launch / 100, 0, 1) * 35
...
def main():
    ...
    for market, rs in by_market.items():
        ...
        if (
            launch >= 55
            and transition["transition_score"] >= 55
    ...
    payload = {
        "version": "Live Ranker v0.5",
        "generated_at_utc": now.isoformat(),
        "lookback_hours": LOOKBACK_HOURS,
        "top_n": TOP_N,
        "method_note": (
            "실험용 비교점수이며 확률이 아님. v0.5는 72시간 상태전이(잠복→재점화→가격확인)를 추적한다. "
