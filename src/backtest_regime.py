...
OUT = Path("docs/data/backtest_regime_latest.json")
...
def main():
    ...
    out = {
        "version": "V6.4 Market Regime Backtest",
        ...
        "results": results,
        "notes": [
            "과적합 방지를 위해 Market Regime 규칙과 GREEN/YELLOW/RED 경계값을 결과 확인 전에 고정",
            "이번 결과를 본 뒤 regime 임계값을 미세조정하지 않는 것을 권장",
            "B점수 공식은 변경하지 않았고, V6.3과 동일하게 score() 내부 btc=2 근사를 유지하여 regime 필터 효과만 분리",
            "V6.4는 결정시각 직전까지 완전히 끝난 캔들만 사용하도록 시간 정렬을 엄격하게 수정",
            "따라서 V6.3 숫자와 단순 1:1 비교하면 안 되며, V6.4의 ALL 행을 새로운 기준선으로 사용",
            ...
            "GREEN이 좋아 보여도 완전히 새로운 OOS/forward 데이터에서 재확인하기 전 실전 우위로 간주하지 않음",
        ],
    }
